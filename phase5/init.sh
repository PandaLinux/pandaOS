...
_list=(kf5-setup dbus libevent sqlite pcre grep shared-mime-info desktop-file-utils ruby qt libdbusmenu-qt phonon vlc phonon-backend-vlc nspr zip js polkit polkit-qt libgudev modem-manager dbus-glib libnl nss libndp network-manager libical bluez uri sgml-common docbook-xml docbook-xsl libxslt boost giflib yasm libjpeg-turbo lmdb kf5-frameworks cairo pango libtiff gdk-pixbuf atk hicolor-icon-theme gtk+2 font-forge at-spi2-core atk at-spi2-atk gtk+3 libpwquality libxkbcommon libsndfile json-c pulse-audio taglib mtdev libinput kf5-plasma) # sddm)
